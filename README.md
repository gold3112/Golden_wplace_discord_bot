...
- **監視リスト管理**: `/watch list` でアクティブな監視を一目で見渡せます（管理者は全ユーザー分を表示可能）
...
6.  **完了！**:
    既存チャンネルの場合は即座に監視が開始されます。ボット作成チャンネルの場合は公開設定を選んで開始します。

## コマンド一覧

### ユーザーコマンド
- `/watch status` - 自分の監視詳細を確認
- `/watch list` - 現在監視中のラベル一覧を表示
- `/watch pause` - 監視一時停止
- `/watch resume` - 監視再開
...
- `/watch list` - サーバー内の全アクティブな監視（所有者・チャンネル）をリスト表示
...
- **1ユーザー3監視**: 1つのサーバーにつき、1ユーザー3つまで監視を持てます。
- **監視間隔**: 5分固定。
