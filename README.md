...
- `/watch create`（またはパネルのモーダル）では、監視名・タイプだけを入力します。チャンネルが自動作成されたら、Botの案内に従って **座標をメッセージで送信** → **テンプレート画像(PNG)を添付** → **`threshold 25` のように通知閾値(px)を入力** してください。
...
- 通知しきい値は `threshold 30` のようにチャンネルへ再投稿するか、`/watch settings threshold <px>` でも変更できます。
